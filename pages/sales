// pages/sales.tsx — Just Autos Sales Dashboard
...
export default function SalesDashboard() {
  ...
  function selectFY(y:number){setFyYear(y);setIsCustomRange(false);setDateLoading(true);setActiveDateParams(`startDate=${y-1}-07-01&endDate=${y}-06-30`)}
  ...
  const load=useCallback(async(isRefresh=false)=>{
    ...
    }catch(e:any){setError(e.message);setLoading(false);setDateLoading(false)}
  ...
  const totalPipeline=filteredQuotes.reduce((s,q)=>{
    let p=0;['3 Days','14 Days','On Hold','Quote On Hold','Quote Sent','Not Done'].forEach(k=>{p+=(q.stats[k]?.value||0)});return s+p
  },0)
  const winRate=(totalWon+totalLost)>0?Math.round((totalWon/(totalWon+totalLost))*100):0

  // Monthly orders sorted
  const monthlyArr=orders?Object.entries(orders.monthly).sort(([a],[b])=>a.localeCompare(b)).map(([k,v])=>{
    const [y,m]=k.split('-');const d=new Date(parseInt(y),parseInt(m)-1);
    return {key:k,label:d.toLocaleDateString('en-AU',{month:'short',year:'2-digit'}),orders:v.orders,value:v.value}
  }):[]
  ...
  return (
    <>
      ...
      <div style={{background:T.bg,minHeight:'100vh',fontFamily:"'DM Sans',system-ui,sans-serif",color:T.text}}>
        ...
        {/* ── TOP BAR ── */}
        <div style={{height:52,background:T.bg2,borderBottom:`1px solid ${T.border}`,display:'flex',alignItems:'center',padding:'0 20px',gap:12}}>
          <a href="/" style={{fontSize:12,color:T.text3,textDecoration:'none',marginRight:4}}>← Portal</a>
          <div style={{width:28,height:28,borderRadius:7,background:'linear-gradient(135deg,#4f8ef7,#a78bfa)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:'#fff'}}>S</div>
          <div style={{fontSize:14,fontWeight:600,flex:1}}>Sales Dashboard</div>

          {/* View toggle */}
          <div style={{display:'flex',gap:2,background:T.bg3,borderRadius:7,padding:2}}>
            {([['workshop','Workshop Sales'],['distributor','Dist. Bookings']] as const).map(([k,l])=>
              <button key={k} onClick={()=>{setView(k);setSubTab('overview')}} style={{padding:'4px 14px',borderRadius:5,border:'none',fontSize:11,fontWeight:view===k?600:400,
          ...
          {[currentFY-1,currentFY].map(y=>
            <button key={y} onClick={()=>selectFY(y)} style={{padding:'3px 10px',borderRadius:4,border:'1px solid',fontSize:11,fontFamily:'monospace',fontWeight:600,cursor:'pointer',
              background:fyYear===y&&!isCustomRange?T.accent:'transparent',color:fyYear===y&&!isCustomRange?'#fff':T.text2,borderColor:fyYear===y&&!isCustomRange?T.accent:T.border}}>
              FY{y}
            </button>
          )}
        ...
        <div style={{padding:20,maxWidth:1400,margin:'0 auto',position:'relative'}}>
          ...
          {loading&&<div style={{display:'flex',alignItems:'center',justifyContent:'center',height:300,flexDirection:'column',gap:12}}>
          ...
          {!loading&&!error&&data&&<>
            ...
            {view==='workshop'&&<>
              ...
              {/* PIPELINE TAB */}
              {subTab==='pipeline'&&<>
                <div style={{display:'grid',gridTemplateColumns:`repeat(${filteredQuotes.length},1fr)`,gap:8,marginBottom:12}}>
