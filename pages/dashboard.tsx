...
export default function Portal({ user }: { user: PortalUserSSR }) {
  ...
  // Sync section with ?s= query param — supports deep links like
  // /dashboard?s=invoices from the sidebar, and lets users share URLs.
  useEffect(() => {
    if (!router.isReady) return
    const q = router.query.s
    const s = Array.isArray(q) ? q[0] : q
    if (s && ['overview','invoices','pnl','stock','payables'].includes(s)) {
      setSection(s as Section)
    }
  }, [router.isReady, router.query.s])

  // When user clicks a section tab in-page, update the URL so refresh/back works
  function handleSectionChange(s: Section) {
    setSection(s)
    router.replace({ pathname: '/dashboard', query: { s } }, undefined, { shallow: true })
  }
