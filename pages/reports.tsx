...
//
// Flow:
//  1. User picks a report type (filtered to ones their role can generate)
//  2. Sets period (default: current calendar month)
//  3. Optionally customises which sections to include
//  4. Clicks "Generate Preview" → POST /api/reports/generate
//  5. Sees live preview rendered in-page
//  6. Clicks "Download PDF" → POST /api/reports/pdf → file saves
...
export default function ReportsPage({ user }: { user: PortalUserSSR }) {
  ...
  const [sectionOverrides, setSectionOverrides] = useState<Partial<Record<SectionId, { periodStart: string; periodEnd: string }>>>({})
  ...
  // The default section list for the currently selected type
  const defaultSections = useMemo<SectionId[]>(() => {
    return selectedType ? DEFAULT_SECTIONS[selectedType] : []
  }, [selectedType])

  // The effective section list (either user's custom selection or the default)
  const effectiveSections = customSections ?? defaultSections

  // Reset custom sections + overrides when report type changes
  useEffect(() => {
  ...
  }, [selectedType, periodStart, periodEnd, entities, effectiveSections, sectionOverrides, title])
  ...
  // Feed chat context: tell the assistant what's on screen so it can answer questions about the preview.
  ...
  if (availableTypes.length === 0) {
    return <>
      ...
      <div style={{ background: T.bg, minHeight: '100vh', color: T.text, marginLeft: 260, padding: 40 }}>
  ...
  return (
    <>
      <Head><title>Reports — Just Autos</title></Head>
      <PortalSidebar activeId="reports" currentUserRole={user.role} currentUserName={user.displayName} currentUserEmail={user.email}/>
      <div style={{ background: T.bg, minHeight: '100vh', color: T.text, marginLeft: 260, padding: '32px 40px', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 16 }}>
                  {(Object.keys(SECTION_META) as SectionId[]).map(sid => {
                    const isDefault = defaultSections.includes(sid)
                    ...
                    return (
                      <label key={sid} style={{
                        ...
                        opacity: isDefault || isChecked ? 1 : 0.62,
                      }}>
                        <input
                          type="checkbox"
                          checked={isChecked}
...
// ── Preview section renderer (simplified HTML/CSS version of the PDF) ──────
