...
const fmtFull=(n:number)=>'$'+Number(n||0).toLocaleString('en-AU',{minimumFractionDigits:0,maximumFractionDigits:0})
...
export default function SalesDashboard(){
  ...
  // Distributor data — filtered by person if selected
      ...
      <div style={{display:'flex',height:'100vh',overflow:'hidden',fontFamily:"'DM Sans',system-ui,sans-serif",color:T.text}}>
        ...
        <div style={{width:220,minWidth:220,background:T.bg2,borderRight:`1px solid ${T.border}`,display:'flex',flexDirection:'column',height:'100vh',overflowY:'auto'}}>
          ...
          <div style={{padding:'14px 10px 4px',flex:1}}>
            ...
            {sidebarNav.map(item=>(
              <a key={item.label} href={item.href} style={{display:'flex',alignItems:'center',gap:9,padding:'8px 10px',borderRadius:7,fontSize:13,marginBottom:1,background:'transparent',color:T.text2,textDecoration:'none'}}>
                <div style={{width:7,height:7,borderRadius:'50%',background:item.color,flexShrink:0}}/><span style={{flex:1}}>{item.label}</span>
              </a>
            ))}
          </div>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:T.bg}}>
          ...
          <div style={{flex:1,padding:20,overflowY:'auto',position:'relative'}}>
            ...
            {!loading&&!error&&data&&<>
              ...
              {view==='workshop'&&<>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
                  <div style={{display:'flex',gap:6}}>
                    {[{k:'overview',l:'Overview'},{k:'orders',l:'Orders'},{k:'pipeline',l:'Quote Pipeline'}].map(t=>
                      <button key={t.k} onClick={()=>setSubTab(t.k)} style={{padding:'6px 18px',borderRadius:6,border:`1px solid ${subTab===t.k?T.accent:T.border}`,fontSize:12,fontWeight:subTab===t.k?600:400,background:subTab===t.k?`${T.accent}18`:'transparent',color:subTab===t.k?T.accent:T.text2,cursor:'pointer',fontFamily:'inherit'}}>{t.l}</button>
                    )}
                  </div>
                  <div style={{display:'flex',gap:4}}>
                    {['All',...quotes.map(q=>q.rep)].map(r=>
                      <button key={r} onClick={()=>setRepFilter(r)} style={{padding:'5px 12px',borderRadius:5,border:`1px solid ${repFilter===r?RC[r]||T.accent:T.border}`,fontSize:11,background:repFilter===r?(RC[r]||T.accent):'transparent',color:repFilter===r?'#fff':T.text3,cursor:'pointer',fontFamily:'inherit'}}>{r}</button>
                    )}
                  </div>
                </div>
                ...
                {subTab==='overview'&&<>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:12,marginBottom:16}}>
                    ...
                    <KPI label="Quote Pipeline" value={fmt(tPipe)} sub={`${tq} quotes`} subColor={T.amber} accent={T.amber}/>
                    <KPI label="Quotes Won" value={String(tWon)} sub={`${wr}% win · ${fmt(tWonVal)}`} subColor={T.green} accent={T.purple}/>
                    <KPI label="Quotes Lost" value={String(tLost)} sub={`${100-wr}% loss`} subColor={T.red} accent={T.red}/>
                    ...
                    <Card>
                      ...
                      {mArr.map((m,i)=><div key={m.key} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                        ...
                        <div style={{flex:1,height:22,background:T.bg4,borderRadius:4,overflow:'hidden',position:'relative'}}>
                          <div style={{height:'100%',borderRadius:4,background:i===mArr.length-1?T.teal:T.blue,width:`${Math.round((m.value/maxM)*100)}%`,opacity:0.85}}/>
                        </div>
                      ...
                      {orders&&Object.entries(orders.byType).sort(([,a],[,b])=>b.value-a.value).map(([type,d])=>{
                        ...
                        return <div key={type} style={{marginBottom:14}}>
                          <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                            <span style={{fontSize:13,color,fontWeight:500}}>{type||'Other'}</span>
                            <span style={{fontSize:12,fontFamily:'monospace',color:T.text2}}>{d.count} · {fmt(d.value)} · {pct}%</span>
                          </div>
                    ...
                    <div style={{overflowX:'auto'}}>
                      <table style={{width:'100%',borderCollapse:'collapse'}}>
                        <thead><tr>{['Rep','Quotes','Won','Lost','Win%','Won $','Pipeline','Sent','3Day','14Day','Hold','RLMNA'].map(h=>
                          <th key={h} style={{fontSize:10,color:T.text3,textTransform:'uppercase',padding:'0 8px 10px',textAlign:h==='Rep'?'left':'right',fontWeight:500,whiteSpace:'nowrap'}}>{h}</th>
                        )}</tr></thead>
                        <tbody>{fq.map(q=>{
                          const w=q.stats['Quote Won']?.count||0,l=q.stats['Quote Lost']?.count||0,r=(w+l)>0?Math.round((w/(w+l))*100):0
                          const pp=['3 Days','14 Days','On Hold','Quote On Hold','Quote Sent'].reduce((s,k)=>s+(q.stats[k]?.value||0),0)
                ...
                {subTab==='orders'&&<>
                  ...
                  <Card>
                    <SH right={<a href="https://just-autos.monday.com/boards/1838428097" target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:T.blue,textDecoration:'none'}}>Open Orders Board →</a>}>Monthly Revenue</SH>
                    {mArr.map((m,i)=><div key={m.key} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                      <span style={{fontSize:12,color:T.text2,width:52,flexShrink:0,fontFamily:'monospace'}}>{m.label}</span>
                      <div style={{flex:1,height:28,background:T.bg4,borderRadius:5,overflow:'hidden',position:'relative'}}>
                        <div style={{height:'100%',borderRadius:5,background:i===mArr.length-1?T.teal:T.blue,width:`${Math.round((m.value/maxM)*100)}%`,opacity:0.85}}/>
                        <span style={{position:'absolute',right:10,top:5,fontSize:12,fontFamily:'monospace',color:'#fff',fontWeight:500}}>{fmt(m.value)}</span>
                      </div>
                ...
                {subTab==='pipeline'&&<>
                  <div style={{display:'grid',gridTemplateColumns:`repeat(${Math.min(fq.length,5)},1fr)`,gap:12,marginBottom:16}}>
                    {fq.map(q=>{
                      const w=q.stats['Quote Won']?.count||0,l=q.stats['Quote Lost']?.count||0,r=(w+l)>0?Math.round((w/(w+l))*100):0
                      const pp=['3 Days','14 Days','On Hold','Quote On Hold','Quote Sent'].reduce((s,k)=>s+(q.stats[k]?.value||0),0)
                      return <Card key={q.rep} style={{borderTop:`3px solid ${RC[q.rep]}`,padding:18}}>
                  ...
                  <Card>
                    ...
                    {STATUSES_ORDER.map(status=>{
                      const count=fq.reduce((s,q)=>s+(q.stats[status]?.count||0),0)
                      const val=fq.reduce((s,q)=>s+(q.stats[status]?.value||0),0)
                      const pct=tq>0?Math.round((count/tq)*100):0
                      ...
                      return <div key={status} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                        ...
                        <div style={{flex:1,height:22,background:T.bg4,borderRadius:4,overflow:'hidden',position:'relative'}}>
                          <div style={{height:'100%',borderRadius:4,background:STATUS_COLORS[status]||T.text3,width:`${pct}%`,opacity:0.8}}/>
                        </div>
              ...
              {/* ═══ DISTRIBUTOR ═══ */}
              {view==='distributor'&&<>
                {/* Person filter for distributor bookings */}
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
                  <Card>
                    <SH right={<a href="https://just-autos.monday.com/boards/1923220718" target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:T.blue,textDecoration:'none'}}>Open Board →</a>}>Revenue by Distributor</SH>
                    {distArr.map(d=><div key={d.name} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                      ...
                      <div style={{flex:1,height:20,background:T.bg4,borderRadius:4,overflow:'hidden'}}>
                        <div style={{height:'100%',borderRadius:4,background:T.blue,width:`${Math.round((d.value/maxDist)*100)}%`,opacity:0.8}}/>
                      </div>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:14}}>
                    <Card>
                      ...
                      {distStatusArr.map(s=><div key={s.status} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:`1px solid ${T.border}`}}>
                        <span style={{fontSize:13,color:T.text2}}>{s.status}</span>
                        <span style={{fontSize:12,fontFamily:'monospace',color:T.text2}}>{s.count} · {fmt(s.value)}</span>
                      </div>)}
                    </Card>
                    <Card>
                      <SH>Period Bookings — {distPersonFilter==='All'?dist?.mtdTotal?.count||0:dist?.mtdByPerson?.[distPersonFilter]?.count||0} · {fmt(distPersonFilter==='All'?dist?.mtdTotal?.value||0:dist?.mtdByPerson?.[distPersonFilter]?.value||0)}</SH>
                      {distMtdArr.slice(0,10).map(d=><div key={d.name} style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:`1px solid ${T.border}`}}>
                        <span style={{fontSize:13,color:T.text2}}>{d.name}</span>
                        <span style={{fontSize:12,fontFamily:'monospace',color:T.green}}>{d.count} · {fmt(d.value)}</span>
                      </div>)}
