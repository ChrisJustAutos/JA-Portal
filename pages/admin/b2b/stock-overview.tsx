...
//
// Alloy restyle 2026-08-12: theme T + kit pills/cards; tile red/amber/green is
// data semantics and maps straight onto A.bad/A.warn/A.good.
...
import { A, RADIUS, SHADOW, Btn, btnStyle, Card, PageTitle, SectionLabel, EmptyState } from '../../../components/b2b/ui'
...
const numInp: React.CSSProperties = { width: 70, padding: '7px 9px', background: T.bg3, border: '1px solid transparent', borderRadius: RADIUS.sm, color: T.text, fontSize: 13, fontFamily: 'inherit', outline: 'none' }
// Coloured threshold dot for the red/amber labels (replaces the emoji).
const Dot = ({ color }: { color: string }) => <span style={{ width: 8, height: 8, borderRadius: RADIUS.pill, background: color, display: 'inline-block', flexShrink: 0 }} />
...
export default function B2BStockOverview({ user }: Props) {
  ...
  function tileColour(t: Item): string {
    ...
    if (q < red) return A.bad
    if (amber != null && q < amber) return A.warn
    return A.good
  }
        ...
        <main className="b2b-admin-main" style={{ flex: 1, padding: '28px 32px', width: '100%', boxSizing: 'border-box' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
            {boards.map(b => {
              ...
              return (
                <button key={b.id} onClick={() => setActiveId(b.id)} className={`al-press al-focus${on ? '' : ' al-ghost'}`}
                  style={{
                    padding: '8px 16px', borderRadius: RADIUS.pill, fontSize: 13, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer',
                    background: on ? T.bg4 : 'transparent', color: on ? T.text : T.text2, border: 'none', boxShadow: on ? SHADOW.sm : 'none',
                  }}>{b.name}</button>
              )
            })}
            {canEdit && <Btn variant="ghost" size="sm" onClick={newView}>New view</Btn>}
          </div>

          <PageTitle
            sub={<>{board ? `${tiles.length} product${tiles.length === 1 ? '' : 's'}` : 'No views yet'}{oldestCache ? ` · updated ${rel(oldestCache)}` : ''}</>}
            action={
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <div style={{ display: 'flex', background: T.bg3, borderRadius: RADIUS.pill, padding: 3 }}>
                      {COLUMN_OPTIONS.map(n => (
                        <button key={n} onClick={() => mutate({ ...config, columns: n })} disabled={!canEdit} className="al-press al-focus"
                          style={{ padding: '6px 11px', minWidth: 34, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: canEdit ? 'pointer' : 'default', border: 'none', borderRadius: RADIUS.pill, background: config.columns === n ? A.accent : 'transparent', color: config.columns === n ? '#fff' : T.text2 }}>{n}</button>
                      ))}
                    </div>
                  </div>
                )}
                <Btn variant="ghost" size="sm" onClick={refresh} disabled={refreshing}>{refreshing ? 'Refreshing…' : 'Refresh'}</Btn>
                {canEdit && board && <Btn variant={configuring ? 'primary' : 'secondary'} size="sm" onClick={() => setConfiguring(v => !v)}>{configuring ? 'Done' : 'Configure'}</Btn>}
              </div>
            }>
            {board?.name || 'Stock Wall'}
          </PageTitle>
          ...
          {configuring && canEdit && board && (
            <Card style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                ...
                <Btn variant="ghost" size="sm" onClick={renameView}>Rename</Btn>
                <span style={{ flex: 1 }} />
                <button onClick={deleteView} className="al-press al-focus al-ghost" style={{ ...btnStyle('ghost', 'sm'), color: A.bad }}>Delete view</button>
              </div>
              <div style={{ display: 'flex', gap: 22, flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: 18 }}>
                <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <span style={{ fontSize: 12, color: T.text2, fontWeight: 650, display: 'inline-flex', alignItems: 'center', gap: 6 }}><Dot color={A.bad} /> Default red below</span>
                ...
                <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <span style={{ fontSize: 12, color: T.text2, fontWeight: 650, display: 'inline-flex', alignItems: 'center', gap: 6 }}><Dot color={A.warn} /> Default amber below</span>
                ...
                <div style={{ fontSize: 12.5, color: T.text3, lineHeight: 1.5, maxWidth: 340 }}>
              ...
              <div className="b2b-col2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
                <div>
                  <SectionLabel>In this view ({tiles.length})</SectionLabel>
                  {tiles.length === 0 && <div style={{ fontSize: 12.5, color: T.text3, fontStyle: 'italic' }}>Nothing added yet — add products from the right.</div>}
                  {tiles.map((t, i) => (
                    <div key={t.id} style={{ padding: '8px 10px', background: T.bg3, borderRadius: RADIUS.sm, marginBottom: 6 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <button onClick={() => moveItem(i, -1)} disabled={i === 0} title="Move up" className="al-press al-focus" style={arrowBtn(i === 0)}>↑</button>
                        <button onClick={() => moveItem(i, 1)} disabled={i === tiles.length - 1} title="Move down" className="al-press al-focus" style={arrowBtn(i === tiles.length - 1)}>↓</button>
                        <button onClick={() => removeItem(t.id)} title="Remove from view" className="al-press al-focus" style={{ ...arrowBtn(false), color: A.bad }}>✕</button>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: T.text3 }}>
                          <Dot color={A.bad} /> <input type="number" min={0} value={t.stock_red_below ?? ''} placeholder={`def ${config.red_below}`}
                            onChange={e => setItemThreshold(t.id, 'red', e.target.value)} onBlur={() => persistItemThreshold(t.id)} style={{ ...numInp, width: 74, background: T.bg2 }} />
                        </label>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: T.text3 }}>
                          <Dot color={A.warn} /> <input type="number" min={0} value={t.stock_amber_below ?? ''} placeholder={config.amber_below != null ? `def ${config.amber_below}` : 'off'}
                            onChange={e => setItemThreshold(t.id, 'amber', e.target.value)} onBlur={() => persistItemThreshold(t.id)} style={{ ...numInp, width: 74, background: T.bg2 }} />
                        </label>
                        <span style={{ fontSize: 12, color: T.text3 }}>blank = default</span>
                      </div>
                    </div>
                  ))}
                </div>
                <div>
                  <SectionLabel>Add a product</SectionLabel>
                  <input value={picker} onChange={e => setPicker(e.target.value)} placeholder="Search SKU or name…" style={{ ...numInp, width: '100%', boxSizing: 'border-box', marginBottom: 8, padding: '9px 11px' }} />
                  <div style={{ maxHeight: 280, overflowY: 'auto' }}>
                    {candidates.map(i => (
                      <div key={i.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderBottom: `1px solid ${T.border}` }}>
                        ...
                        <span style={{ fontSize: 12, color: T.text3, fontFamily: 'monospace', fontVariantNumeric: 'tabular-nums' }}>{i.is_inventoried ? i.qty_on_hand : '∞'}</span>
                        <button onClick={() => addItem(i.id)} className="al-press al-focus al-ghost" style={{ ...btnStyle('ghost', 'sm'), color: A.accent }}>Add</button>
                      </div>
                    ))}
                    {candidates.length === 0 && <div style={{ fontSize: 12.5, color: T.text3, padding: '8px 0', fontStyle: 'italic' }}>No matches.</div>}
                  </div>
                </div>
              </div>
              {savedAt && <div style={{ fontSize: 12.5, color: A.good, fontWeight: 600, marginTop: 10 }}>✓ Saved</div>}
            </Card>
          )}
          ...
          {loading ? (
            ...
            <EmptyState title="No views yet" sub={canEdit ? 'Create a view to group tiles (e.g. Airboxes, Exhausts).' : undefined}
              action={canEdit ? <Btn onClick={newView}>New view</Btn> : undefined} />
          ) : tiles.length === 0 ? (
            <EmptyState title="No products in this view yet" sub={canEdit ? 'Use Configure to add some.' : undefined}
              action={canEdit ? <Btn onClick={() => setConfiguring(true)}>Configure</Btn> : undefined} />
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${config.columns}, minmax(0, 1fr))`, gap: 12 }}>
              {tiles.map(t => {
                ...
                return (
                  <div key={t.id} style={{ background: `${c}14`, border: `1.5px solid ${c}66`, borderRadius: RADIUS.md, padding: '16px 14px', display: 'flex', flexDirection: 'column', minHeight: 96 }}>
                    <div style={{ fontSize: 12, color: T.text3, fontFamily: 'monospace', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.sku}</div>
...
function arrowBtn(disabled: boolean): React.CSSProperties {
  return { width: 28, height: 28, borderRadius: RADIUS.pill, border: 'none', background: T.bg2, color: disabled ? T.text3 : T.text2, cursor: disabled ? 'default' : 'pointer', fontSize: 13, fontFamily: 'inherit', opacity: disabled ? 0.5 : 1, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }
}
