...
export default function DistributorReport({ user }: { user: PortalUserSSR }) {
  ...
  function renderPartsTunes(){
    ...
    return <div style={{padding:24,overflowY:'auto',display:'flex',flexDirection:'column',gap:16}}>
      ...
      {/* Per-model section — table, or the month × model chart. The chart is
          three-dimensional: distributor (the selector above), one series per
      ...
      {(chartKinds['pt-models']||'table')!=='table' && (()=>{
        ...
        if(kind==='pie'){
          ...
          return <ChartBox ckey="pt-models" config={{type:'pie',data:{labels:rows.map(r=>r.model),datasets:[{data:rows.map(r=>Math.round(r.parts*10)/10),backgroundColor:CHART_COLORS,borderWidth:0}]},options:{...baseOpts,plugins:{...baseOpts.plugins,legend:{position:'right',labels:{color:T.text2,font:{size:11}}},title:{display:true,text:partsLabel,color:T.text3}}}}}/>
        }
        const measure=chartKinds['pt-measure']||'tunes'
        const months=monthKeysOf(ptLines)
        const mmOf=(l:LineItem)=>(l.Date||'').slice(0,7)
        const modelsList=rows.map(r=>r.model)

        // Tunes: distinct VINs per model per month.
        const tSets=new Map<string,Map<string,Set<string>>>()
        for(const l of ptLines){
          if(!isTuneJob(l))continue
          const m=tuneModelOf(l); if(!m)continue
          const ym=mmOf(l); if(!ym)continue
          if(!tSets.has(m))tSets.set(m,new Map())
          const mo=tSets.get(m)!
          if(!mo.has(ym))mo.set(ym,new Set())
          mo.get(ym)!.add((l.poNumber||'').trim().toUpperCase())
        }
        const tunesAt=(m:string,ym:string)=>tSets.get(m)?.get(ym)?.size||0

        // Parts per model per month — same semantics as the table's lens.
        const pCount=new Map<string,Map<string,number>>()
        const addP=(m:string,ym:string,v:number)=>{
          if(!pCount.has(m))pCount.set(m,new Map())
          pCount.get(m)!.set(ym,(pCount.get(m)!.get(ym)||0)+v)
        }
        if(ptMode==='units'){
          for(const l of ptLines){
            if(l.bucket!=='Parts')continue
            ...
            const units=l.qty!=null&&l.qty>0?l.qty:1
            const w=tuneWeights(pm.models)
            pm.models.forEach((m,i)=>addP(m,ym,units*w[i]))
          }
        }else{
          const invInfo=new Map<string,{ym:string,spec:Set<string>,amb:Set<string>}>()
          for(const l of ptLines){
            if(l.bucket!=='Parts')continue
            const pm=partModelsOf(l); if(pm.excluded)continue
            const inv=l.invoiceNumber||`${l.CustomerName}|${l.Date}`
            if(!invInfo.has(inv))invInfo.set(inv,{ym:mmOf(l)||'?',spec:new Set(),amb:new Set()})
            const e=invInfo.get(inv)!
            if(!pm.models.length)continue
            if(pm.models.length===1)e.spec.add(pm.models[0])
            else pm.models.forEach(m=>e.amb.add(m))
          }
          invInfo.forEach(e=>{
            e.spec.forEach(m=>addP(m,e.ym,1))
            const overlaps=Array.from(e.amb).some(m=>e.spec.has(m))
            const leftover=Array.from(e.amb).filter(m=>!e.spec.has(m))
            if(!overlaps&&leftover.length){const w=tuneWeights(leftover);leftover.forEach((m,i)=>addP(m,e.ym,w[i]))}
          })
        }
        const partsAt=(m:string,ym:string)=>Math.round((pCount.get(m)?.get(ym)||0)*10)/10

        const valAt=(m:string,ym:string):number|null=>{
          if(measure==='tunes')return tunesAt(m,ym)
          if(measure==='parts')return partsAt(m,ym)
          const t=tunesAt(m,ym)
          return t>0?Math.round((partsAt(m,ym)/t)*100):null
        }
        const datasets=modelsList.map((m,i)=>({
          label:m,
          data:months.map(ym=>valAt(m,ym)),
          borderColor:CHART_COLORS[i%CHART_COLORS.length],
          backgroundColor:kind==='line'?'transparent':CHART_COLORS[i%CHART_COLORS.length],
          borderRadius:3,tension:0.3,pointRadius:3,pointBackgroundColor:CHART_COLORS[i%CHART_COLORS.length],
          spanGaps:true,
        }))
        const measureLabel=measure==='tunes'?'Tunes (distinct VINs)':measure==='parts'?(ptMode==='cars'?'Cars with parts':'Parts units'):'Parts coverage %'
        return <>
          <div style={{fontSize:11,color:T.text3}}>{distLabel()} · {measureLabel} per month, one series per model{measure==='%'?' — months with no tunes are skipped':''}</div>
          <ChartBox ckey="pt-models-monthly" height={400} config={{
            type:kind==='bar'?'bar':'line',
            data:{labels:months,datasets},
            options:{...baseOpts,plugins:{...baseOpts.plugins,legend:{labels:{color:T.text2,font:{size:11}}},tooltip:{callbacks:{label:(ctx:any)=>`${ctx.dataset.label}: ${ctx.raw}${measure==='%'?'%':''}`}}},
              scales:{x:{grid:{color:'rgba(var(--t-ink),0.05)'},ticks:{color:T.text3}},y:{grid:{color:'rgba(var(--t-ink),0.05)'},ticks:{color:T.text3,callback:(v:any)=>measure==='%'?v+'%':v}}}},
          }}/>
        </>
      })()}
