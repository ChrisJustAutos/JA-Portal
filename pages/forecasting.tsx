...
// Forecast by month — reads the current Mechanics Desk job WIP report and
// shows expected revenue per calendar month, based on Job Date being today
// or later and a Total value > 0 on the job.
//
// The data is auto-refreshed nightly via Pipeline C (graph-jobreport-mail
// webhook). Manual upload remains available at /settings → Data Imports
// as a fallback.
//
// 30 Apr 2026: Renamed from /jobs → /forecasting. The /jobs route now
// redirects here. Sidebar tab id stays 'jobs' (permission key view:jobs)
// to avoid breaking user_profiles.visible_tabs rows.
...
interface Report { id: string; uploaded_at: string; filename: string | null; row_count: number; notes: string | null; source?: string | null }
          ...
          {loading ? (
            ...
            <div style={{background:T.bg2, border:`1px dashed ${T.border2}`, borderRadius:12, padding:60, textAlign:'center'}}>
              ...
              <div style={{fontSize:13, color:T.text3, marginBottom:20}}>
                The nightly Mechanics Desk WIP report will auto-import at ~6:10 PM each evening.
                <br/>Or upload one manually now to populate the forecast.
              </div>
