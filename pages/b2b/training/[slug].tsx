...
// Training player: slide viewer (per-section stepper, keyboard arrows, resume
// from where you left off) → quiz (one question per screen) → results with
// full per-question review. Marking is server-side — this page never sees a
// correct answer until the attempt has been submitted and recorded.
...
interface ExamResultData { score_pct: number; passed: boolean; pass_pct: number; review: ReviewQ[] }

type Mode = 'slides' | 'exam' | 'result'

const slideKey = (slug: string) => `ja-b2b-training-slide:${slug}`
const pad2 = (n: number) => String(n).padStart(2, '0')

export default function B2BTrainingPlayer({ b2bUser }: Props) {
  ...
  const load = useCallback(async () => {
    const r = await fetch(`/api/b2b/training/${encodeURIComponent(slug)}`, { credentials: 'same-origin' })
  ...
  useEffect(() => {
    ...
    load()
      .then(d => {
        ...
        try {
          const saved = parseInt(localStorage.getItem(slideKey(slug)) || '', 10)
  ...
  const goTo = useCallback((idx: number) => {
    ...
    setSlideIdx(clamped)
    try { localStorage.setItem(slideKey(slug), String(clamped)) } catch {}
  }, [slug, totalSlides])
  ...
  async function submitExam() {
    ...
    try {
      ...
      const r = await fetch(`/api/b2b/training/${encodeURIComponent(slug)}`, {
  ...
  return (
    <>
      <Head><title>{data?.module.title || 'Training'} · Just Autos B2B</title><meta name="robots" content="noindex,nofollow" /></Head>
      <B2BLayout user={b2bUser} active="training">
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>

          {/* ── Header ─────────────────────────────────────────── */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <a href="/b2b/training" style={{ fontSize: 12, color: T.text3, textDecoration: 'none', whiteSpace: 'nowrap' }}>← All courses</a>
          ...
          {/* ── Results ────────────────────────────────────────── */}
          {mode === 'result' && result && (
            <ResultScreen result={result} onRetake={retake} onJumpToSlide={jumpToSlide} />
          )}
        </div>
      </B2BLayout>
    </>
...
// ─── Results + review ─────────────────────────────────────────────────────
function ResultScreen({ result, onRetake, onJumpToSlide }: {
