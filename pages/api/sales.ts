  ...
  const data = await mondayQuery(`{
    boards(ids: [${boardId}]) {
      items_page(limit: 500) {
        items {
          column_values(ids: ["status", "numeric_mkzcbhz2"]) { id text }
        }
      }
    }
  }`)
  ...
  const data = await mondayQuery(`{
    boards(ids: [${ORDERS_BOARD}]) {
      items_page(limit: 500, query_params: {
        ...
        items { column_values(ids: ["date", "numbers", "color_mks9wfk9"]) { id text } }
      }
    }
  }`)
  const items = data?.boards?.[0]?.items_page?.items || []
...
async function getDistributorBookings(startDate: string, endDate: string) {
  ...
  for (const item of items) {
    ...
    byDistributor[dist].count++; byDistributor[dist].value += val
    if (!byStatus[status]) byStatus[status] = { count: 0, value: 0 }
    byStatus[status].count++; byStatus[status].value += val
    if (!byPerson[person]) byPerson[person] = { count: 0, value: 0 }
    byPerson[person].count++; byPerson[person].value += val
    total.count++; total.value += val

    if (dateStr >= startDate && dateStr <= endDate) {
      if (!mtdByDist[dist]) mtdByDist[dist] = { count: 0, value: 0 }
      mtdByDist[dist].count++; mtdByDist[dist].value += val
      if (!mtdByPerson[person]) mtdByPerson[person] = { count: 0, value: 0 }
      mtdByPerson[person].count++; mtdByPerson[person].value += val
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  return requireAuth(req, res, async () => {
    ...
    const result = { fetchedAt: new Date().toISOString(), period: { startDate, endDate }, orders: ordersData, distributors: distData, quotes: quoteResults.filter(Boolean) }
