...
interface InventoryItem {
  ...
  marginPct: number | null           // (sellEx - avgCost) / sellEx, null if no sell price
  marginDollar: number | null        // sellEx - avgCost, null if no sell price
  reorderLevel: number               // min level before alert
  reorderQty: number                 // suggested order qty
  ...
  revenue90d: number                 // ex-GST, approx using avg sell price ex GST
  lastSoldDate: string | null
  daysSinceLastSold: number | null   // null if never sold
  runRatePerDay: number              // units per day over last 90 days
  daysOfCover: number | null         // qtyOnHand / runRate; null if no velocity
  stockoutDate: string | null        // ISO date when stock hits 0 at current run rate
  stockoutStatus: 'out' | 'critical' | 'low' | 'ok' | 'dead' | 'noSales'

  // flags
  isLowStock: boolean                // qtyOnHand <= reorderLevel and reorderLevel > 0
  isOutOfStock: boolean              // qtyOnHand <= 0
  isDead90d: boolean                 // stockValue > 0 and unitsSold90d == 0
  isDead180d: boolean                // stockValue > 0 and no sales in 180d
}

interface TotalsBlock {
  totalItems: number                 // active inventoried
  totalSkus: number                  // same as totalItems for now
  stockValue: number
  qtyOnHand: number
  qtyOnOrder: number
  qtyCommitted: number
  ...
  reorderSuggestCount: number        // items below reorder level with reorder qty set
  reorderSuggestValue: number        // suggested reorder cost @ avg cost
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // NOTE: SaleInvoices has no TaxCodeCode at header level — tax code lives on lines.
    ...
    // CData can't filter by parent Date here, so we pull everything that joins
    // to one of our invoices-with-date. This is typically a few thousand rows.
    ...
    // ── 4) Aggregate per-item velocity from lines that match our invoices ─
    interface Agg {
      units30: number; units90: number; units365: number
      revenue90Ex: number            // ex GST
      lastSold: Date | null
    ...
    for (const line of lines) {
      ...
      // tax code lives on the line itself (invoice header has none)
