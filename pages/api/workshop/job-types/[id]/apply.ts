...
// POST { booking_id } — copy a job type's template lines onto a booking
// (appended after any existing lines). Optionally stamps the booking's
// job_type. Gated edit:bookings.
...
export default withAuth('edit:bookings', async (req, res) => {
  ...
  const db = sb()
  const { data: jobType } = await db.from('workshop_job_types').select('id, code, name').eq('id', jobTypeId).maybeSingle()
