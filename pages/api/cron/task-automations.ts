// pages/api/cron/task-automations.ts
// Vercel cron (5-min) — walks due Tasks automation flows (graph engine).
// Auth: CRON_SECRET bearer (or the vercel-cron user-agent), like the other crons.
import { NextApiRequest, NextApiResponse } from 'next'
import { processDueTaskAutomations } from '../../../lib/task-automations'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await processDueTaskAutomations(150)
