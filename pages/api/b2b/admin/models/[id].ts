// pages/api/b2b/admin/models/[id].ts
//
// PATCH  → update name / sort_order / is_active
// DELETE → remove the model. Catalogue items that referenced it are
...
export default withAuth('edit:b2b_distributors', async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'PATCH') {
    ...
    const { data, error } = await c.from('b2b_models').update(update).eq('id', id).select().single()
    if (error) {
  ...
  if (req.method === 'DELETE') {
    const { error } = await c.from('b2b_models').delete().eq('id', id)
