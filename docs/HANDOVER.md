...
- **Charts show every customer, grouped, with a combined TOTAL per section and a grand total at the end** (Chris 2026-08-21). `chartRows` builds that: customers ordered within their section, a `— TOTAL` row after each section, then `ALL COMBINED — TOTAL` when there is more than one section. The per-customer detail is the point — "so you can see accurately what each made" — so do not collapse it to one bar per section; that was tried and rejected. Category stacking (Tuning/Parts/Oil + custom) still applies within each bar.
  - **Pie and doughnut drop the total rows** (`chartRows.filter(r => !r.isTotal)`): a total slice would be counted twice and the percentages would stop meaning anything. Any new chart type must make the same choice.
  - On National Total the total bars are drawn in purple so they read as a rule-off rather than another customer.
