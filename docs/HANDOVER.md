...
- **A palletised order is CARTONISED FIRST, then the cartons are stacked (2026-08-27).** The original pallet path never boxed anything: it emitted `ceil(totalWeight / max_weight_g)` pallets, shared the weight evenly, and listed every SKU as flat contents. Two things followed. (1) Pallet choice was blind to dimensions — with two pallets on the same weight cap the tie-break went to the *smaller deck*, so a Hunter Mechanical cart holding 1650 mm exhausts was quoted on an 1100×1100 deck it physically overhangs. (2) Pallet count was weight-only, so 2.34 m³ of light parts was declared as one pallet (129% of that pallet's envelope) because 289 kg sat under a 400 kg cap. `packItems()` now runs the carton packer first, then bin-packs those cartons onto pallet slots capped by **both** weight and usable cube (`PALLET_FILL` 0.80, deck height less `PALLET_BASE_MM` 150 for the timber). **The slot count IS the pallet count** — it is no longer computed. A carton that no orientation will place on the deck (`fitsDeck`) comes back `loose` and ships beside the pallets as its own Carton item rather than disqualifying the pallet; pallet selection then compares pallets + loose units, tie-breaking on deck area as before, which is what now rules the small deck out. Each pallet carries its REAL weight, not an even share. Declared height is still `max_height_mm` — an under-declared pallet gets re-cubed at the depot, so over-declaring a short stack is the safe direction and remains a known cost (§9).
...
- Migrations `148` and `153` are duplicated numbers; latest applied is `207_b2b_machship_company_id`, so next is `208`.
...
- **A palletised consignment is declared at the pallet's full `max_height_mm`**, whatever the stack actually measures, so a short pallet over-declares its cube and the distributor is quoted high. Deliberate: under-declaring gets the pallet re-cubed at the depot at JA's expense. Now that the packer knows the real carton stack (2026-08-27) a truer height is computable — worth doing only alongside a check that the layer maths is not optimistic.
- **The pallet cube bound is a volume bound, not a 3D placement.** `PALLET_FILL` 0.80 against the deck envelope is deterministic and defensible, but it cannot know that three 1650 mm exhausts leave awkward strips unusable. It will under-count pallets for an order of long, irregular items that happen to be light. The deck-fit test (`fitsDeck`) is per-carton, not per-layer.
