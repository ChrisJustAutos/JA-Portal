...
**Still open:** the older **"Mark as shipped"** button does no MYOB work at all — it was how 000050 shipped, which is why the order also has no tax invoice. The money now lands regardless, but the Order→Invoice conversion still depends on **Ship Now** or a manual conversion in MYOB.
...
- **"Mark as shipped" bypasses MYOB entirely.** It stamps `shipped_at` and does no conversion, so an order shipped that way has no tax invoice in MYOB and no MachShip manifest — only **Ship Now** does that work. `B2B-2026-000050` went out this way. Since 2026-08-31 the payment still reaches MYOB (against the Sale Order, or the invoice if one was made by hand), but the invoice itself does not.
