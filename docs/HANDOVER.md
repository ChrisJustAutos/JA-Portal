...
- **Recordings still have a size ceiling, now ~100 MB (about 105 minutes).** Recordings are uploaded as raw 8 kHz mono WAV, so length translates directly into megabytes, and `ja-transcribe` downloads from the bucket — a recording that won't upload gets no audio, no transcript, no score and no Slack card. The project-level upload cap was raised from 50 MB to 200 MB on 2026-08-21 (it had rejected the 61:20 Dom call with `413 EntityTooLarge`), so the binding limit is now the `call-recordings` bucket's own 100 MB. Transcoding to MP3/Opus before upload would remove the ceiling entirely and cut recording storage several-fold — the bucket already accepts `audio/mpeg`, `audio/ogg` and `audio/opus`, and a 61-minute call is ~7–14 MB compressed. Not done.
