...
| A stocktake row shows a **negative** system QTY (e.g. −1) when the part is really at zero | Up to 27 Aug 2026 the match read MD's *available* figure (on hand MINUS what's allocated to jobs) for any part not currently in stock, so a zero-stock part with one unit on a job read −1 | Fixed — the count now always uses MD's on-hand quantity. If you still see one on an OLD count sheet, press **Run Match** to refresh it. A negative QTY on a part that really is oversold (sold before it arrived) is genuine and is left alone. |
