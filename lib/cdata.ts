...
function authHeader() {
  const creds = btoa(`${CDATA_USER}:${CDATA_PAT}`)
  return {
...
export async function cdataQuery(_catalog: string, sql: string) {
  ...
  if (!res.ok) throw new Error(`CData MCP ${res.status}: ${await res.text()}`)
  ...
  if (!dataLine) {
    // Fallback: try parsing as plain JSON (some responses aren't SSE)
    try {
