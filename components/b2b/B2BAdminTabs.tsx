...
// Odoo app's own menu. Icon + label pills (Alloy kit), the active one
// highlighted. Also mounts <AlloyStyles/> once so every admin page gets
// the kit's hover/press/focus classes without needing B2BLayout.
//
...
export default function B2BAdminTabs({ active }: { active: B2BAdminSection }) {
  return (
    <nav className="b2b-admin-tabs" style={{
      ...
      {/* Every /admin/b2b page renders this component, so it's the single home
          for the kit's global interaction styles on the admin side. */}
      <AlloyStyles/>
      ...
      <style>{`
        ...
        @media (max-width: 640px){
          ...
          table.b2b-cards td::before{ content:attr(data-label); color:${T.text3}; font-size:12px; font-weight:500; text-align:left; flex-shrink:0; white-space:nowrap; }
      ...
      {TABS.map(tab => {
        ...
        return (
          <a key={tab.id} href={tab.href}
            className={`al-press al-focus${on ? '' : ' al-ghost'}`}
            style={{
              display: 'inline-flex', alignItems: 'center', gap: 7,
              padding: '8px 14px', borderRadius: RADIUS.pill, textDecoration: 'none',
              fontSize: 13, fontWeight: on ? 600 : 500, whiteSpace: 'nowrap', flexShrink: 0,
              color: on ? T.text : T.text2,
              background: on ? T.bg3 : 'transparent',
            }}>
            <span style={{ display: 'flex', color: on ? A.accent : T.text3 }}><AppIcon name={tab.icon} size={17}/></span>
            {tab.label}
          </a>
        )
      })}
